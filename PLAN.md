...
- **LICENSE: GPL-3.0-or-later**, whole repo including the extension. The stated goals are community trust and preventing closed-source forks — that is the GPL's job description and the norm for this category (KeePassXC, Bitwarden). Permissive licensing would maximize `keel-crypto` reuse as a library, but core-as-embeddable-library is not a product goal and permissive invites the "our secure fork, now proprietary" outcome. Pair with **DCO sign-off and no CLA**: contributors keep copyright, which means nobody — including the maintainer — can relicense proprietary later. That irreversibility is itself a trust signal worth advertising.
