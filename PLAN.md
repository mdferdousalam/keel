# Bitting — Open-Source, Post-Quantum-Resistant Password Manager
...
| 1 — bitting-crypto, bitting-format, bitting-hardening | **Done** | ~34M fuzz executions after the last format change, zero crashes |
| 2 — bitting-store, bitting-core | **Done** | Crash safety verified by SIGKILL; policy engine complete including the approval lifecycle |
| 3 — bitting-proto, bitting-agent, bitting-client, bitting-cli | **Done** | Full CLI surface |
| 3 — bitting-reveal | **Done** | winit + softbuffer, hand-built bitmap font, no font parser. Excluded from screen capture on macOS, verified. The agent spawns it and pipes the secret, so plaintext never enters the GUI process. |
...
| 5 — Browser extension | **Done, minus pairing** | `bitting-native-host`, MV3 extension, `bitting setup-browser`. Origin matching in the agent, with every look-alike case tested. SAS pairing and the Noise channel are not built. |
...
- **Pairing between the extension and the agent.** SAS code plus a Noise channel, per §6.3.
  ...
  threat model the rest of Bitting is written against — recorded as a gap, not skipped quietly.
- **Windows — it does not compile.** `bitting-hardening` references windows-sys symbols that do
...
4. **No Public Suffix List.** The plan called for it. It is genuinely needed to derive a
   *registrable domain from a request origin* — without it `bank.co.uk` and `evil.co.uk` both
   reduce to `co.uk`. Bitting asks the opposite question: does this **stored** origin, which the
...
6. **Escalations are raised without a GUI attached.** The plan failed closed by refusing to
   ask when no window was open. Once `bitting approvals` existed, that told users on headless
...
11. **The exhaustive `match` on `Response` in bitting-mcp caught five omissions** as variants
...
6. **A killed agent stranded the user.** `wait_for_socket` waited for the socket file to
   *exist*, which a socket left behind by a SIGKILL satisfies instantly — so the client
   spawned an agent, declared the socket ready, connected, and was refused. Every `bitting`
...
7. **A second agent could hijack a running agent's socket.** `Listener::bind` unlinked any
   ...
   `BITTING_AGENT_SOCKET` is user-settable and a typo should not delete a real file. The
...
9. **`audit_tail` is not exposed as an MCP tool**, though §7.2 contemplated it under an
   ...
   an agent, and `bitting log` now covers the user's own need. The rogue-agent test deliberately
   does not assert either way.
10. **The exhaustive `match` on `Response` in bitting-mcp caught three omissions** as response
...
Product name: **`bitting`** (CLI binary `bitting`, reverse-DNS `dev.bitting`). A bitting is the pattern of cuts on a key — the code that reproduces the thing which opens the lock, which is what this product stores.

> **Renamed from `keel`, August 2026, and the reason is a lesson.** This line used to read "Renaming is a global find/replace — do the trademark/crates.io check in Phase 0 before writing code." The check was listed as a Phase 0 deliverable and never done. By the time it ran, `keel-cli` and `keel-core` were both taken on crates.io by an active project of the same name, which broke `cargo install keel-cli` — a stated distribution channel — and there were two further Keels in developer tooling. The rename cost ~1,900 substitutions across 125 files plus a vault-format break, all of it cheap only because there are no signed releases and no real vaults yet. The advice in the original line was right; it just needed doing. `bitting` was checked first this time: whole crate namespace free, no OS package, no company, no loaded meaning.

---

## 1. Architecture

**Single hardened agent** (the ssh-agent / 1Password model). One long-lived `bitting-agent` process per user session owns the unlocked vault. Everything else — GUI, CLI, MCP server, browser native-messaging host — is a thin client over authenticated local IPC. Unlock happens once; key material lives in exactly one hardened process.

```
 bitting (CLI) ──────UDS──►┌──────────────────────────────────────┐
 bitting-mcp ────────UDS──►│  bitting-agent  (only process with keys)│──► vault.bitting
 bitting-native-host ─────►│  · VMK + subkeys (mlock'd, zeroized) │──► vault.audit
    ▲ stdio             │  · session table + per-client policy │──► vault.state
 browser extension      │  · grant/approval engine             │
                        │  · autolock, clipboard auto-clear    │
 Bitting.app (Tauri) ─────►│  · spawns bitting-reveal for plaintext  │
   └─ approval modals   └──────────────────────────────────────┘
```

**The invariant that makes review tractable: only `bitting-agent` links `bitting-crypto`/`bitting-format` at runtime.** Every other process is a pipe for already-authorized, mostly-masked data. CI enforces this (`cargo xtask check-layering`).

### Transport and peer authentication

- **Unix:** socket in a `0700` directory — `$XDG_RUNTIME_DIR/bitting/agent.sock` (Linux), `~/Library/Application Support/dev.bitting/agent.sock` (macOS); socket mode `0600`. Verify peer UID via `SO_PEERCRED` (Linux) / `LOCAL_PEERCRED` + `LOCAL_PEERPID` (macOS); require `uid == geteuid()`.
- **Windows:** named pipe `\\.\pipe\dev.bitting.agent-<user-sid-hash>` with a DACL granting only the current user SID, `PIPE_REJECT_REMOTE_CLIENTS`, plus `GetNamedPipeClientProcessId` + image signature check for audit attribution.
- **Base wire protocol:** length-prefixed (`u32` LE) JSON frames of `bitting-proto` enums, versioned hello, 1 MiB cap, fuzzed parser. JSON rather than postcard here because the native-messaging side is already JSON and debuggability matters for IPC (the *on-disk* format uses postcard — see §3).
...
- **Spawn:** `bitting-client` connect-or-spawns the agent (found next to own executable, else `$PATH`), guarded by a lockfile against races. Agent exits after N minutes idle-and-locked. launchd/systemd-user units ship in `packaging/` as opt-in.
...
- **State broadcast:** clients subscribe to `StateChanged{locked|unlocked}` — tray icon, extension badge, and `bitting status` all read one truth.
...
├── crates/
│   ├── bitting-crypto/        # KDF tiers+calibration, HKDF namespace, XChaCha20 AEAD, SecretBytes,
│   │                       #   CSPRNG, password/passphrase generator. #![forbid(unsafe_code)], no I/O
│   ├── bitting-format/        # on-disk binary codec: header/manifest/record, padding, AAD, DoS guards.
│   │                       #   Pure, fuzzed. THE frozen contract everything depends on.
│   ├── bitting-hardening/     # THE ONLY crate allowed `unsafe`: mlock, core-dump suppression,
│   │                       #   ptrace deny, capture exclusion, panic wipe
│   ├── bitting-store/         # atomic write transaction, advisory locking, backup rotation, OS paths
│   ├── bitting-core/          # vault open/save, records, autolock state machine, hash-chained audit log,
│   │                       #   policy/grant engine (the single allow/deny/ask chokepoint)
│   ├── bitting-proto/         # IPC wire types only (serde). Parallel leaf, no logic.
│   ├── bitting-agent/         # bin `bitting-agent` — the daemon; the only runtime holder of keys
│   ├── bitting-client/        # connect-or-spawn + typed API. NO crypto deps.
│   ├── bitting-reveal/        # bin `bitting-reveal` — tiny native (winit) non-capturable overlay window
│   │                       #   that renders one plaintext secret. No webview. ~300 lines.
│   ├── bitting-cli/           # bin `bitting` (clap, rpassword)
│   ├── bitting-mcp/           # bin `bitting-mcp` (rmcp, stdio) — stateless, policy-free proxy
│   ├── bitting-native-host/   # bin `bitting-native-host` — browser stdio ⇄ agent proxy. Dumb pipe.
│   ├── bitting-import/        # features: csv, chromium, firefox. Only crate touching rusqlite/DPAPI/NSS.
│   └── bitting-breach/        # OPTIONAL HIBP client — the ONLY crate allowed a TLS/HTTP dep. Off by default.
├── apps/desktop/           # Tauri v2 app: src-tauri/ (Rust shell → bitting-client) + ui/ (TS + Svelte, vite)
...
```
bitting-crypto  → crypto crates, zeroize, subtle, getrandom       (no I/O, no clock, no rand injection needed beyond getrandom)
bitting-format  → bitting-crypto, postcard, serde                    (pure; parses attacker-controlled bytes)
bitting-store   → bitting-format
bitting-core    → bitting-crypto, bitting-format, bitting-store, bitting-hardening
bitting-proto   → serde only
bitting-agent   → bitting-core, bitting-proto, snow, tokio
bitting-client  → bitting-proto                                      ← crucially NOT bitting-crypto
bitting-cli / bitting-mcp / bitting-native-host / desktop → bitting-client (+ bitting-import for cli & desktop)
...
```bash
cargo tree -e normal -p bitting-core --prefix none \
...
```
"bitting/v1/index"      "bitting/v1/record/" || record_id || LE32(key_epoch)
"bitting/v1/audit"      "bitting/v1/attach/" || attach_id
"bitting/v1/pairing-root"                 "bitting/v1/search"   (reserved)
```
...
ikm = BLAKE3::keyed_hash(
        key  = blake3("bitting/v1/prekey" || vault_uuid),
...
### 3.6 On-disk format (`vault.bitting`)

```
┌ HEADER (plaintext, fully covered by AAD) ───────────────────────────────┐
│ magic[8]="BITTVLT\x01" · format_version u16 · header_len u32 · flags u32
...
A_wrap     = "bitting/v1/wrap"     || vault_uuid || H || LE32(epoch)
A_manifest = "bitting/v1/manifest" || vault_uuid || H || LE64(write_counter) || LE16(format_version)
A_record   = "bitting/v1/record"   || vault_uuid || H || record_id || LE32(key_epoch)
```
...
**Parse guards (all before any allocation):** `header_len ≤ 64 KiB`, `manifest_len ≤ 64 MiB`, `records_len ≤ 4 GiB`, per-record `ct_len ≤ 16 MiB`, entries ≤ 500k, every offset bounds-checked. `bitting-format::decode` is **the single highest-value fuzz target in the project** — it is the only code parsing fully attacker-controlled bytes.
...
1. Take an exclusive lock on `vault.bitting.lock`; fail fast with "another instance has this vault open".
...
3. Write `vault.bitting.tmp.<rand>` with mode `0600` / current-user-only DACL.
4. `file.sync_all()`.
5. Rotate backups: `vault.bitting` → `.bak.1`, shifting `.1`→`.2`→`.3` (keep 3, each tagged with its `write_counter`).
6. `fs::rename(tmp, vault.bitting)` — atomic on the same filesystem, including Windows.
...
**This vault's confidentiality path contains zero public-key cryptography.** Password → Argon2id → symmetric key → AEAD. There is no key exchange to record and no long-term asymmetric secret whose future compromise unlocks past ciphertexts. An attacker who harvests `vault.bitting` today and waits 20 years is in exactly the position they are in today. The correct one-line statement is: **"harvest now, decrypt later" is a public-key problem, and this vault has no public-key crypto in it.** The residual risk is entirely "was the password guessable given 20 years of classical compute?" — which is why we enforce entropy floors, offer hardware factors, and let KDF params be raised on an existing file.
...
- Lints: `#![forbid(unsafe_code)]` everywhere except `bitting-hardening`; in crypto/format/core also `#![deny(clippy::print_stdout, print_stderr, dbg_macro, unwrap_used, expect_used, indexing_slicing)]`.
...
### Process hardening (`bitting-hardening`, called before any secret exists)
...
3. Decrypted secrets are **never cloned**. `use_secret`-style operations consume the buffer inside `bitting-agent` and return only a status.
...
- **"Reveal" spawns `bitting-reveal`** — a tiny native winit window (no webview) that receives the secret from the agent over an inherited socket, renders it in a non-capturable window (`WDA_EXCLUDEFROMCAPTURE` / `sharingType = .none`), and wipes on close.
...
page (untrusted) ─ content script (isolated world) ─ background SW (MV3)
                                                          │ nativeMessaging
                                                          ▼
                                        bitting-native-host (dumb pipe)
                                                          │ Noise_KKpsk0 over UDS / named pipe
                                                          ▼
                                                    bitting-agent (holds keys)
```
The native host does framing translation and nothing else — no keys, no decryption. If the agent isn't running it returns `app_not_running`; it must **never** silently launch the app and prompt for the master password (that's a phishing vector). The extension shows "open Bitting" instead.
...
- **Zero third-party JS dependencies** — plain ES modules, no build-time supply chain. `lib/protocol.ts` is generated from a JSON Schema exported from `bitting-proto` (`cargo xtask gen-proto-schema`), so the wire format has one source of truth.
...
3. **The GUI shows a modal with the extension id, the browser's process path + code-signature identity, and a 6-digit pairing code.** The user reads it from Bitting and types it into the extension popup — SAS-style, binding both UIs to the same human and defeating a background process racing to pair.
...
`bitting-mcp` is a separate process (stdio, `rmcp` — the official Rust MCP SDK) with **no keys and no vault file access**. It connects to the agent over the same Noise-authenticated channel as the extension, and every request passes through `bitting-core`'s **policy engine** — the single allow/deny/ask chokepoint shared by MCP, extension, and CLI. If the agent isn't running or the vault is locked, everything fails closed with `vault_locked`. Launch config:
...
9. Launch-time enforcement flags: `bitting-mcp --client-id X --read-only --no-reveal --allow-tags 'work/*' --ttl 15m`.
...
**Password generator** (`bitting-crypto::generator`):
...
- **Budget: ≤150 crates** in `bitting-core`'s normal-dependency tree. Every addition needs written justification in the PR and a `cargo-vet` audit or trusted import.
...
- **`cargo-auditable`** for all shipped binaries so users and distros can run `cargo audit bin bitting` on the artifact they actually downloaded.
...
- `unsafe` forbidden outside `bitting-hardening`; `cargo-geiger` reported in CI for dependency awareness.
...
- **macOS:** cross-compile with `cargo-zigbuild` or an osxcross container for determinism. `codesign` embeds a timestamp, so publish **both** hashes — `bitting-macos-arm64.unsigned.sha256` (reproducible, rebuildable by anyone) and the ad-hoc-signed `.dmg` hash — and document the `codesign --remove-signature` comparison procedure.
...
- **Primary channel: a Homebrew tap** (`brew install --cask bitting`, `brew install bitting` for the CLI). Homebrew installs are not browser-quarantined, so Gatekeeper's "damaged app" dialog never appears. This is also the path most technical macOS users already prefer.
...
**Linux** (no gatekeeping at all — this will be the smoothest platform): `.deb` + `.rpm` via `nfpm`, AppImage, Flatpak, and an AUR PKGBUILD (`bitting` from source, `bitting-bin`), each with detached minisign signatures. Pursue Debian/Fedora packaging later — distro maintainers become independent reviewers, which is a security asset.

**Everywhere:** `cargo install bitting-cli` builds from source with a committed lockfile.
...
**v1-essential:**
- **`bitting-crypto` / `bitting-format` unit tests** — every path; run under **Miri** weekly (pure crates make this viable, and it catches UB in any `unsafe` zeroize glue).
- **Known-answer tests** — `crates/bitting-format/tests/vectors/*.json` with RFC 8439 (ChaCha20-Poly1305) and RFC 9106 (Argon2) vectors plus our own format vectors. These double as the cross-implementation conformance suite and the "format is frozen" contract.
...
- **Integration** — spawn the agent on a temp socket (`BITTING_AGENT_SOCKET` override), drive via `bitting-client`; CLI via `assert_cmd` for init→unlock→add→get→lock plus locked/denied exit codes, on all three OSes in CI.
...
```
bitting init [--vault <path>] [--tier interactive|balanced|paranoid]
bitting unlock [--timeout 15m]        bitting lock        bitting status [--json]
bitting add <name> [--username u] [--url u] [--generate | --password-stdin]
bitting get <name|id> [--field password|username|otp|notes] [--clip | --show] [--json]
bitting list [--folder f] [--json]    bitting search <query> [--json]
bitting edit <name|id> [--field f --value-stdin]        bitting rm <name|id> [--yes]
bitting generate [--length 24 | --words 6] [--no-symbols] [--clip]
bitting import <file> --format chrome-csv|firefox-csv|bitwarden|1password|keepassx|generic-csv
bitting import --browser chrome|edge|firefox
bitting export [--format json|csv] [--output -]     # fresh passphrase re-entry + loud warning
bitting audit [--json]                # weak/reused/old, offline Bloom filter
bitting agent [--foreground]          bitting mcp       bitting setup-browser
bitting verify-release <dir>          bitting completions <shell>
```

**Secret handling:**
- **No secret ever in argv.** No `--password <p>` flag exists anywhere (argv is world-readable via `ps` and lands in shell history). Inputs: interactive TTY prompt (`rpassword`), `--password-stdin` / `--value-stdin`, or `BITTING_PASSPHRASE_FILE` pointing at a `0600` file for automation. **No plain env-var passphrase** — env leaks via `/proc/<pid>/environ` and into CI logs.
- **Safe-by-default `get`:** with a TTY and no flags, `bitting get x` copies to the clipboard with a 30 s **agent-performed** auto-clear (so `bitting` can exit immediately) and prints nothing secret. `--show` is required to print; when stdout is a pipe, printing is allowed for scripting but `--show`/`--field` must still be explicit.
...
| **1 — crypto + format** | `bitting-crypto` (Argon2id tiers + calibration, HKDF namespace, XChaCha20 wrappers, `SecretBytes`, generator); `bitting-format` (header/manifest/record codec, padding, AAD, DoS guards); `bitting-hardening` (mlock, core-dump suppression, panic wipe) | KATs pass; proptest round-trip + tamper-any-byte properties hold; **72 h fuzz on `vault_parse` with zero panics**; `docs/vault-format.md` written and the format declared frozen |
| **2 — store + core** | `bitting-store` full atomic-write transaction; `bitting-core` open/save, rollback/truncation detection, autolock state machine, hash-chained audit log, **policy/grant engine** (built now — the extension needs it too, not just MCP) | Crash-simulation tests pass; rollback-detection scenarios covered; policy engine unit-tested against every scope/grant/limit path |
| **3 — agent + CLI** | `bitting-proto`, `bitting-agent` (IPC, peer checks, sessions, Noise, autolock, clipboard auto-clear), `bitting-client`, full CLI of §11, `bitting-reveal` overlay | End-to-end init→unlock→add→get→lock on 3 OSes in CI; `ipc_frame` fuzzer live; **maintainer dogfooding daily as their real password manager** |
...
- **LICENSE: AGPL-3.0-or-later**, whole repo including the extension, with two carve-outs. The stated goals are community trust and preventing closed-source forks — that is copyleft's job description and the norm for this category (KeePassXC, Bitwarden). Pair with **DCO sign-off and no CLA**: contributors keep copyright, which means nobody — including the maintainer — can relicense proprietary later. That irreversibility is itself a trust signal worth advertising.
  ...
  - **Two crates are permissive, and the layering rule is what makes that safe.** `bitting-proto` is `Apache-2.0` (wire types only — nothing to protect, and everything that speaks to the agent needs it; Apache rather than MIT for the express patent grant). `bitting-client` is `MPL-2.0` (third parties embed it; per-file copyleft is the narrowest thing that permits that). Both are crates the dependency rules already prove cannot reach key material, the crypto core, or the vault format. Note this rules out a permissive `bitting-format`, which depends on `bitting-crypto`. Enforced by `cargo xtask check-licenses`, not by memory.
  ...
  - **Trademark, separately** (`TRADEMARK.md`). The licence deliberately permits proprietary-*service* competitors to fork; what stops a rebranded clone trading on the audit story is the name, which no licence covers. "Bitting" is an obscure locksmithing term applied to software, which makes it an *arbitrary* mark — the strongest class short of an invented word, and a large improvement on the previous name. Caveat recorded there: still unregistered, and the USPTO register itself could not be searched (bot protection), so this is the absence of a visible conflict rather than a clean register.
...
- **CONTRIBUTING.md:** dev setup, `cargo xtask` commands, the dependency budget rule, the **no-asymmetric-without-hybrid rule**, DCO, conventional commits, and a note that security-sensitive areas (`bitting-crypto`, `bitting-format`, `bitting-agent` auth paths) carry deliberately slower review latency.
...
3. **End-to-end (Phase 3):** `bitting init` → `unlock` → `add` → `get --show` → `lock` → `get` returns exit code 2, driven by `assert_cmd` in CI on macOS, Linux, and Windows. Then use it as your actual password manager for two weeks before Phase 4.
4. **GUI invariant (Phase 4):** the integration test that intercepts every Tauri command result and IPC frame during a scripted session (unlock, view entry, copy, reveal) and asserts no known secret byte sequence appears in any of them. Manually confirm `bitting-reveal` renders as a separate native window and that a screenshot of it comes back blank on macOS and Windows.
...
7. **Release integrity (Phase 7):** on a clean machine that never had the source, download the release, run `bitting verify-release` and confirm it **fails** when you tamper with one byte of any artifact and when either signature is removed. Independently run `rebuild.yml` on a fork and diff the Linux musl hashes against the published `SHA256SUMS`.
