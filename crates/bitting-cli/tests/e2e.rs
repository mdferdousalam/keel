...
//! End-to-end tests driving the real `bitting` binary against a real agent.
//!
...
/// An isolated Bitting installation for one test.
struct Fixture {
...
impl Fixture {
    ...
    fn bitting(&self) -> Command {
        let mut command = Command::new(Self::binary("bitting"));
        command
            .env("BITTING_AGENT_SOCKET", &self.socket)
            .env("BITTING_VAULT", &self.vault)
            .env("BITTING_AGENT_BINARY", Self::binary("bitting-agent"))
            ...
            .env("BITTING_AGENT_IDLE_EXIT_SECS", "5")
    ...
    fn run(&self, args: &[&str]) -> Output {
        self.bitting().args(args).output().expect("run bitting")
    }
...
impl Drop for Fixture {
    fn drop(&mut self) {
        ...
        //
        // Only if a socket is actually there: `bitting lock` would otherwise *spawn* an agent
        ...
        if self.socket.exists() {
            let _ = self.bitting().arg("lock").output();
        }
    }
}
...
fn locking_makes_reads_fail_with_a_distinct_exit_code() {
    ...
    assert!(
        stderr(&output).contains("bitting unlock"),
...
fn the_wrong_passphrase_is_refused_without_saying_which_factor_was_wrong() {
    ...
    let output = fixture
        .bitting()
        .env("BITTING_PASSPHRASE_FILE", &wrong)
        .arg("unlock")
        .output()
        .unwrap();
...
fn json_output_is_machine_readable_for_every_read_command() {
    ...
    for args in [
        ...
        serde_json::from_str::<serde_json::Value>(&out).unwrap_or_else(|e| {
            panic!(
                "`bitting {}` produced invalid JSON: {e}\n{out}",
                args.join(" ")
            )
        });
    }
...
fn no_secret_appears_in_the_agent_audit_log() {
    ...
    let audit = fixture.path().join("vault.bitting.audit");
    if audit.is_file() {
...
fn write_stdin(mut command: Command, input: &str) -> Output {
    ...
    child
        ...
        .expect("write stdin");
    child.wait_with_output().expect("wait for bitting")
}
...
fn the_audit_chain_survives_locking_and_unlocking() {
    ...
    // would open `bitting log` and be told their audit log had been tampered with.
    //
...
fn removing_records_from_the_end_of_the_audit_log_is_detected() {
    ...
    let path = fixture.vault.with_extension("bitting.audit");
...
fn a_killed_agent_does_not_strand_the_user() {
    ...
    // immediately declared the socket ready, connected, and was refused. Every `bitting`
