...
//! * **Exit codes are stable** and defined in `bitting-proto`, so scripts can branch on
...
use bitting_client::{Client, Error as ClientError};
use bitting_proto::{
...
const CLIENT_ID: &str = "bitting-cli";
...
#[command(
    ...
    long_about = "Bitting keeps your passwords in one encrypted file on this machine.\n\
                  ...
                  history. For automation, point BITTING_PASSPHRASE_FILE at a file with mode \
...
enum Command {
    ...
    Unlock {
        ...
        ///
        /// Only pass this if you know you restored a backup. See `bitting status`.
        #[arg(long)]
    ...
    Grant {
        ...
        /// BITTING_MCP_CLIENT_ID, defaulting to "bitting-mcp".
        client_id: String,
    ...
    ///
    /// Safer than `bitting get --show`, which leaves the password in terminal scrollback.
    Show {
        /// Entry name or id.
        name: String,
        /// Which field.
    ...
    /// Firefox how to start `bitting-native-host`, and which extensions may do so. Needs no vault
...
fn report(error: &ClientError) {
    eprintln!("bitting: {error}");
    match error.code() {
        Some(ErrorCode::Locked) => eprintln!("hint: run `bitting unlock` first."),
        Some(ErrorCode::NoVault) => eprintln!("hint: run `bitting init` to create a vault."),
        Some(ErrorCode::Conflict) => {
            eprintln!("hint: another Bitting instance changed the vault. Retry the command.");
        }
        _ => {}
    }
}
...
const NATIVE_HOST_NAME: &str = "dev.bitting.native_host";

/// Filename of the bridge binary, as installed next to `bitting`.
#[cfg(windows)]
const HOST_BINARY: &str = "bitting-native-host.exe";
/// Filename of the bridge binary, as installed next to `bitting`.
#[cfg(not(windows))]
const HOST_BINARY: &str = "bitting-native-host";
...
const DEFAULT_EXTENSION_ID: &str = "bittingvaultbittingvaultbittingva";

/// Firefox add-on ID. Firefox matches on this rather than on a per-machine identifier.
const FIREFOX_EXTENSION_ID: &str = "bitting@bitting.dev";
            ...
            if running != absolute.as_path() {
                return Err(ClientError::Refused(format!(
                    "an agent is already running for {}, so --vault {} cannot be used.\n\
                     Run `bitting lock` and let that agent retire, or point BITTING_AGENT_SOCKET at \
...
fn import(
    ...
    let report = bitting_import::read_csv(file).map_err(|error| ClientError::Agent {
    ...
    for entry in &report.entries {
        ...
        match client.request(&request) {
            ...
            Err(error) => {
                ...
                eprintln!("bitting: could not import {:?}: {error}", entry.title);
            }
        }
    }
    ...
    if shred_after {
        match bitting_import::shred(file) {
            Ok(()) => {
                if !json {
                    println!(
            ...
            Err(error) => eprintln!("bitting: could not delete the import file: {error}"),
        }
    } else if !json {
        // The file is now the most dangerous thing on the disk. Say so.
        println!("\n{}", bitting_import::EXPORT_WARNING);
    }
    ...
    if reveal_requested {
        ...
        println!(
            "\nNote: `reveal` lets the agent ask for plaintext passwords. Each request still \
             needs your approval in the Bitting window, and reveal is disabled for AI agents \
...
///
/// Standard output is allowed but nudged away from, because `bitting export > file` creates
/// the file with the shell's umask — commonly world-readable — before Bitting sees it.
fn export(
    client: &mut Client,
    format: &str,
    ...
    match output {
        ...
        None => {
            ...
            print!("{body}");
            eprintln!(
                "\n{} entr{} exported. Nothing was written to disk by Bitting.",
                entries.len(),
                plural(entries.len())
            );
        }
...
/// link here.
fn render_export_csv(entries: &[bitting_proto::ExportedEntry]) -> String {
...
fn write_private_file(path: &std::path::Path, bytes: &[u8]) -> Result<(), ClientError> {
    ...
    let mut file = options.open(path).map_err(|e| ClientError::Io {
        context: if e.kind() == std::io::ErrorKind::AlreadyExists {
            "creating the export file: something already exists at that path, and Bitting \
...
fn show(client: &mut Client, name: &str, field: &str, json: bool) -> Result<(), ClientError> {
    ...
    emit(
        json,
        "Opened a Bitting window showing it. It closes after 30 seconds, or on any key.",
...
/// such program can execute `bitting-native-host` directly, or talk to the agent's socket
...
fn setup_browser(extension_id: Option<&str>, dry_run: bool, json: bool) -> Result<(), ClientError> {
    ...
    let host_path = std::env::current_exe()
        ...
        .ok_or_else(|| {
            ClientError::Unexpected("could not work out where bitting-native-host is".into())
        })?;
    if !host_path.exists() {
        eprintln!(
            "bitting: {} is not next to this binary. The manifests below will point at it \
             anyway, so re-run this once it is installed alongside `bitting`.",
            host_path.display()
        );
    }
    ...
    let chromium = serde_json::json!({
        ...
        "description": "Bitting browser bridge",
        "path": host_path,
        "type": "stdio",
        ...
        if cfg!(not(unix)) {
            println!(
                "Registering the browser bridge on Windows means writing a registry value \
                 under\nHKCU\\Software\\<browser>\\NativeMessagingHosts, which Bitting does not \
    ...
    println!(
        "This lets a browser start Bitting's bridge. It does not grant access to your vault:\n\
...
/// nothing here writes system-wide, which would need privileges Bitting should never ask for.
        ...
        if enabled {
            ...
            eprintln!(
                "AI agents may now be shown plaintext secrets.\n\
                 ...
                 \n\
                 Turn it back off with:  bitting settings --agent-reveal off"
            );
        } else {
...
fn approvals(
    ...
    for item in &approvals {
        ...
        println!(
            "  Answer with:  bitting approvals --allow {}   (or --refuse)",
            item.approval_id
        );
        println!();
    }
...
fn log(client: &mut Client, limit: u32, json: bool) -> Result<(), ClientError> {
    ...
    match chain {
        ...
        bitting_proto::ChainState::Intact => {}
    }
    ...
    if matches!(chain, bitting_proto::ChainState::Intact) {
...
fn audit(client: &mut Client, json: bool) -> Result<(), ClientError> {
    ...
    if flagged == 0 {
        ...
        println!("Rotate one with:  bitting rotate <name>");
    }
...
fn get(
    ...
    // asked for the value on purpose, but they still have to say --show, so a stray `bitting
    ...
    if std::io::stdout().is_terminal() {
        eprintln!("bitting: printing a secret to a terminal; it will remain in scrollback.");
    }
...
///
/// The supported way to run Bitting non-interactively. A file is used rather than an
...
/// permissions.
const PASSPHRASE_FILE_ENV: &str = "BITTING_PASSPHRASE_FILE";
...
/// to stdin when it is not a terminal is what makes `bitting` usable in a script without
...
fn print_json(value: &serde_json::Value) {
    match serde_json::to_string_pretty(value) {
        ...
        Err(error) => eprintln!("bitting: could not render JSON: {error}"),
    }
}
