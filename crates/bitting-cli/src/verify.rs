...
//!
//! This is the point at which a user actually places trust. Everything else in Bitting protects
...
pub(crate) enum VerifyError {
    ...
    #[error(
        "this build of Bitting has no release signing keys compiled in, so it cannot verify a \
...
mod tests {
    ...
    fn a_well_formed_manifest_parses() {
        let manifest = format!(
            "{}  bitting-linux-x86_64.tar.gz\n{}  bitting-macos-arm64.dmg\n",
            "a".repeat(64),
            "b".repeat(64)
        );
        let parsed = parse_manifest(manifest.as_bytes()).unwrap();
        assert_eq!(parsed.len(), 2);
        assert!(parsed.contains_key("bitting-linux-x86_64.tar.gz"));
    }
