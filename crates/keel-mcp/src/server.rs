...
//!   startup because no agent was running would make Keel look broken.
...
//!   agent's dream. If no agent is running, the answer is to tell the user to open Keel.

use std::io::{BufRead, Write};

use keel_client::Client;
use keel_proto::ClientKind;
...
/// wants your password".
const DEFAULT_CLIENT_ID: &str = "keel-mcp";

/// Environment variable overriding the client identifier.
pub const CLIENT_ID_ENV: &str = "KEEL_MCP_CLIENT_ID";
        ...
        if self.agent.is_none() {
            let client =
                Client::connect_existing(ClientKind::Mcp, &self.client_id).map_err(|error| {
                    format!(
                        "Keel is not running, so the vault cannot be reached ({error}). Ask the \
                         user to open the Keel app or run `keel unlock`. Do not retry until they \
                         confirm they have."
                    )
                })?;
        ...
        match request.method.as_str() {
            "initialize" => Some(Response::ok(
                ...
                serde_json::json!({
                    ...
                    "instructions":
                        "Keel is a local password manager. Call vault_status first. To log the \
            ...
            Err(error) => {
                ...
                Response::ok(
                    ...
                    to_value(&ToolResult::failure(format!(
                        "lost the connection to Keel ({error}). Ask the user to check that Keel \
                         is running."
                    ))),
                )
            }
