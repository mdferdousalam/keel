...
impl Fixture {
    ...
    fn store_canary(&self) {
        ...
        assert!(
            child.wait().expect("keel add").success(),
    ...
    fn first_pending(&self) -> Option<serde_json::Value> {
        let json = self.keel(&["approvals", "--json"]);
...
fn one_approval_permits_exactly_one_reveal() {
    ...
    let fixture = Fixture::new();
    fixture.keel(&["settings", "--agent-reveal", "on"]);
    ...
    // The user allows it.
    fixture.keel(&["approvals", "--allow", &id]);
...
fn refusing_denies_the_request() {
    ...
    let id = pending
        ...
        .expect("an approval id")
        .to_owned();
    fixture.keel(&["approvals", "--refuse", &id]);
...
fn turning_the_setting_back_off_takes_effect_immediately() {
    ...
    let fixture = Fixture::new();
    fixture.keel(&["settings", "--agent-reveal", "on"]);
    fixture.keel(&["settings", "--agent-reveal", "off"]);
