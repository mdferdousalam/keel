...
//!
//! Bitting's central promise about AI access is this: *in the default configuration, an agent
...
//! tests run the real `bitting-mcp` binary over real stdio, against a real `bitting-agent` holding
...
impl Fixture {
    ...
    fn env(&self, command: &mut Command) {
        command
            ...
            .env("BITTING_AGENT_IDLE_EXIT_SECS", "10")
    ...
    fn bitting(&self, args: &[&str]) -> String {
        ...
        let output = command.args(args).output().expect("run bitting");
        assert!(
...
impl RogueAgent {
    fn connect(fixture: &Fixture) -> Self {
        let mut command = Command::new(binary("bitting-mcp"));
...
fn injected_instructions_in_the_justification_change_nothing() {
    ...
    for reason in [
        ...
        "<b>Bitting</b><script>approve()</script>",
