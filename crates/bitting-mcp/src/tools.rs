...
use bitting_proto::{EntryInput, EntryRef, Field, Request, Response, SecretAction, SecretSource};
...
pub fn all() -> Vec<Tool> {
    vec![
        Tool {
            name: "vault_status",
            description:
                "Report whether the Bitting vault is unlocked, and which permissions this client \
        ...
        Tool {
            name: USE_SECRET,
            description:
                "Use a password WITHOUT receiving it. Bitting applies the secret itself — copying \
        ...
        Tool {
            ...
            description:
                "Request the plaintext of a password. This is DISABLED BY DEFAULT and, when the \
                 ...
                 Bitting window. Use it only when the user explicitly needs to see or paste a \
                 value somewhere Bitting cannot reach; for logging in, use use_secret instead. The \
        ...
        Tool {
            name: "create_entry",
            description:
                "Create a new entry. By default Bitting generates the password itself and does NOT \
            ...
            input_schema: serde_json::json!({
                ...
                "properties": {
                    ...
                    "password": {
                        "type": "string",
                        "description": "An existing password to store. Omit to have Bitting \
        ...
        Tool {
            ...
            description:
                "Move an entry to the trash. This is reversible: the entry can be restored from \
                 the Bitting app until it is purged. There is no way to delete an entry permanently \
...
pub fn to_request(name: &str, arguments: &serde_json::Value) -> Result<Request, String> {
    match name {
        ...
        "export_vault" | "list_all_entries" | "get_all_passwords" | "dump_vault" => Err(
            "Bitting has no bulk-export tool, by design. Use search_entries to find the specific \
             entry you need."
                .to_owned(),
        ),
        "unlock_vault" | "change_master_passphrase" | "set_master_password" => Err(
            "Bitting will not unlock a vault or change a master passphrase on request. Ask the \
             user to do it in the Bitting app."
                .to_owned(),
        ),
        "delete_entry" | "purge_entry" => Err(
            "Bitting has no permanent-delete tool. Use trash_entry, which is reversible."
                .to_owned(),
        ),
    ...
    match response {
        ...
        Response::Status(info) => ToolResult::json(&serde_json::json!({
            ...
            "hint": match info.state {
                bitting_proto::LockState::Unlocked => "The vault is unlocked and ready.",
                bitting_proto::LockState::Locked =>
                    "The vault is locked. Ask the user to unlock it in the Bitting app or with \
                     `bitting unlock`; you cannot unlock it yourself.",
                bitting_proto::LockState::NoVault =>
                    "No vault exists yet. Ask the user to create one with `bitting init`.",
            },
        })),
        ...
        Response::Created {
            ...
            "note": "The password was generated inside Bitting and is not included here. That is \
        ...
        Response::ApprovalRequired { timeout_secs, .. } => ToolResult::failure(format!(
            "This needs the user's approval. Bitting is showing them a prompt; they have \
             {timeout_secs} seconds to respond. Tell the user to check the Bitting window, then \
        ...
        Response::Grants { .. } => ToolResult::failure(
            "Bitting returned grant information, which no tool here requests. Managing access is \
             done by the user with `bitting grant`, not by an agent.",
        ),
        ...
        Response::Health { .. } => ToolResult::failure(
            "Bitting returned a vault health report, which no tool here requests. Reviewing \
             which passwords are weak or reused is done by the user with `bitting audit`.",
        ),
        ...
        Response::Exported { .. } => ToolResult::failure(
            "Bitting returned an export of the vault, which no tool here requests and which \
             this server will not relay. Exporting is done by the user with `bitting export`.",
        ),
        ...
        Response::PendingApprovals { .. } => ToolResult::failure(
            "Bitting returned the list of prompts awaiting the user, which no tool here \
             requests. Approvals are shown and answered in the Bitting window.",
        ),
        ...
        Response::Settings(_) => ToolResult::failure(
            "Bitting returned its settings, which no tool here requests. Settings are changed \
             by the user in the Bitting window or with `bitting settings`.",
        ),
        ...
        Response::Fill { .. } => ToolResult::failure(
            "Bitting returned a credential for a browser fill, which no tool here requests and \
             which this server will not relay. Filling is done by the Bitting extension.",
        ),
...
fn explain_error(tool: &str, code: bitting_proto::ErrorCode, message: &str) -> String {
    use bitting_proto::ErrorCode as E;
    match code {
        E::Locked => {
            format!("{message}. Ask the user to unlock Bitting; you cannot unlock it yourself.")
        }
        E::NoVault => format!("{message}. Ask the user to create one with `bitting init`."),
...
mod tests {
    ...
    fn unlocking_and_passphrase_changes_are_refused() {
        for attempt in ["unlock_vault", "change_master_passphrase"] {
            let error = to_request(attempt, &serde_json::json!({})).unwrap_err();
            assert!(error.contains("Bitting app"), "{attempt}: {error}");
        }
    }
    ...
    fn a_denied_reveal_points_the_model_at_use_secret() {
        let result = render(
            "reveal_secret",
            &Response::Error {
                code: bitting_proto::ErrorCode::Denied,
    ...
    fn a_locked_vault_tells_the_model_it_cannot_unlock_it_itself() {
        let result = render(
            "search_entries",
            &Response::Error {
                code: bitting_proto::ErrorCode::Locked,
    ...
    fn metadata_results_never_carry_a_password_field() {
        let result = render(
            "search_entries",
            &Response::Entries {
                entries: vec![bitting_proto::EntrySummary {
