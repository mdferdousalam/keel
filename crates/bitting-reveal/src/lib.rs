...
//!
//! Bitting's desktop window is a webview, and a webview is a browser: a garbage-collected heap
...
//! implemented** — see [`bitting_hardening::platform`] — so on Windows this window is an ordinary
...
pub fn read_request<R: Read>(reader: &mut R) -> Result<Reveal, String> {
    ...
    if buffer.is_empty() {
        return Err("nothing was piped in; this process is started by Bitting, not by hand".into());
    }
