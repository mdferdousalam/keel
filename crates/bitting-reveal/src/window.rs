...
fn exclude_from_capture(window: &Window) -> bool {
    ...
    // `bitting_hardening` takes a handle with a lifetime rather than a raw pointer, so there is no
    // precondition for this crate — which forbids `unsafe` — to have to vouch for.
    let Ok(handle) = window.window_handle() else {
        return false;
    };
    bitting_hardening::platform::exclude_window_from_capture(handle)
}
...
impl ApplicationHandler for Overlay {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let attributes = WindowAttributes::default()
            ...
            .with_title(format!("Bitting — {}", self.reveal.label))
        ...
        eprintln!(
            "bitting-reveal: hidden from screen capture: {}",
...
pub fn run() -> Result<(), String> {
    ...
    if let Some(reason) = overlay.closed_by {
        eprintln!("bitting-reveal: closed ({reason})");
    }
