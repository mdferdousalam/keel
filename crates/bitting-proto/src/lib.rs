...
//! Wire types for talking to the Bitting agent.
//!
...
//! them; keeping it a leaf is what lets `bitting-client` be provably free of key material.
//!
...
pub enum Request {
    ...
    /// [`bitting_core::origin`] in the agent for what they are.
    ///
    ...
    ///
    /// The **agent** spawns `bitting-reveal` and pipes the value to it, so the plaintext never
...
///
/// Lets the tray icon, the extension badge, and `bitting status` all reflect one truth
...
mod tests {
    ...
    fn requests_round_trip() {
        let requests = vec![
            Request::Hello {
                ...
                client_id: "bitting-cli".to_owned(),
    ...
    fn events_round_trip() {
        let event = Event::ApprovalRequested {
            ...
            executable: Some("/usr/local/bin/bitting-mcp".to_owned()),
