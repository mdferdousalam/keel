...
use bitting_crypto::{aead, Key256};
use bitting_format::codec::{Reader, Writer};
...
impl Outcome {
    ...
    ///
    /// Stable because it appears in `bitting log --json`, so changing one of these strings
...
impl AuditRecord {
    ...
    fn decode_body(bytes: &[u8]) -> Result<Self> {
        ...
        if version == 0 || version > AUDIT_VERSION {
            return Err(Error::Format(bitting_format::Error::Malformed(
        ...
        let client_type = client_type_from_code(r.u8()?).ok_or(Error::Format(
            bitting_format::Error::Malformed("audit record names an unknown client type"),
        ))?;
        let outcome = Outcome::from_code(r.u8()?).ok_or(Error::Format(
            bitting_format::Error::Malformed("audit record names an unknown outcome"),
        ))?;

        let client_id_len = r.u8()? as usize;
        let client_id = String::from_utf8(r.take(client_id_len)?.to_vec()).map_err(|_| {
            Error::Format(bitting_format::Error::Malformed(
    ...
    fn chain_hash(&self) -> Result<[u8; 32]> {
        ...
        hasher.update(b"bitting/v1/audit-chain");
...
impl AuditLog {
    ...
    pub fn append(&mut self, event: &AuditEvent<'_>) -> Result<AuditRecord> {
        ...
        framed.len_u32(
            bitting_crypto::NONCE_LEN + sealed.ciphertext.len(),
...
pub enum ChainIntegrity {
    ...
    /// [`AuditAnchor`](bitting_format::manifest::AuditAnchor).
    ///
...
pub fn read_log(key: &Key256, bytes: &[u8]) -> Result<AuditReport> {
    ...
    if r.array::<8>()? != AUDIT_MAGIC {
        return Err(Error::Format(bitting_format::Error::BadMagic));
    }
    let version = r.u16()?;
    if version == 0 || version > AUDIT_VERSION {
        return Err(Error::Format(bitting_format::Error::UnsupportedVersion {
            found: version,
    ...
    while !r.is_empty() {
        ...
        let nonce = fr.array::<{ bitting_crypto::NONCE_LEN }>()?;
...
pub fn check_against_anchor(
    report: &AuditReport,
    anchor: Option<bitting_format::manifest::AuditAnchor>,
) -> ChainIntegrity {
...
mod tests {
    use super::*;
    use bitting_crypto::SecretBytes;
    ...
    fn editing_a_record_breaks_the_chain_at_that_point() {
        ...
        bytes[cursor + 4 + bitting_crypto::NONCE_LEN + 1] ^= 0xFF;
    ...
    fn truncation_is_classified_as_truncation_at_every_cut_point() {
        ...
        let anchor = bitting_format::manifest::AuditAnchor {
            seq: 5,
