...
use bitting_crypto::kdf::{Argon2Params, KdfTier};
use bitting_crypto::{aead, subkeys, Factors, Key256, SecretBytes};
use bitting_format::header::{Fido2Factor, WrappedKey, YubikeyFactor};
use bitting_format::manifest::{EntryMeta, Id, Manifest, VaultSettings};
use bitting_format::vault::{self as fmt_vault, RecordBlob, VaultImage};
...
/// Domain string for the wrapped-key associated data is owned by `bitting-format`; this
...
pub struct UnlockFactors {
    ...
    pub passphrase: bitting_crypto::SecretString,
...
impl UnlockFactors {
    ...
    #[must_use]
    pub fn passphrase(passphrase: bitting_crypto::SecretString) -> Self {
        Self {
            passphrase,
            keyfile: None,
    ...
    fn factor_set(&self) -> FactorSet {
        FactorSet {
            keyfile: self.keyfile.as_deref().map(bitting_crypto::hash_keyfile),
...
pub struct OpenOptions {
    ...
    ///
    /// Supplied by the caller because `bitting-crypto` performs no I/O and cannot
...
impl UnlockedVault {
    ...
    pub fn create(
        ...
        bitting_crypto::fill_random(&mut vault_uuid)?;
        let mut kdf_salt = [0u8; bitting_crypto::SALT_LEN];
        bitting_crypto::fill_random(&mut kdf_salt)?;
        ...
        let mut header = Header {
            ...
            vault_uuid,
            created_at,
            kdf_id: bitting_crypto::KDF_ID_ARGON2ID_V13,
            kdf_params: params,
            kdf_salt,
            measured_kdf_ms,
            factors: factors.factor_set(),
            aead_id: bitting_crypto::AEAD_ID_XCHACHA20POLY1305,
            ...
            wrapped_keys: vec![WrappedKey {
                epoch: FIRST_EPOCH,
                nonce: [0; bitting_crypto::NONCE_LEN],
                ciphertext: [0; bitting_format::header::WRAPPED_KEY_CT_LEN],
            }],
            write_counter: 1,
            records_offset: 0,
            records_len: 0,
    ...
    pub fn open(
        ...
        let rollback = bitting_store::state::check(last_seen.as_ref(), &header, footer_hash);
    ...
    #[must_use]
    pub const fn audit_anchor(&self) -> Option<bitting_format::manifest::AuditAnchor> {
    ...
    /// [`AuditAnchor`](bitting_format::manifest::AuditAnchor). The caller is the agent,
    ...
    pub fn set_audit_anchor(&mut self, anchor: bitting_format::manifest::AuditAnchor) {
        let advances = self
            .manifest
            .audit_anchor
    ...
    pub fn add_entry(&mut self, draft: EntryDraft, body: &RecordBody) -> Result<Id> {
        ...
        bitting_crypto::fill_random(&mut record_id)?;
        let timestamp = now()?;
    ...
    pub fn trash_entry(&mut self, id: &Id, retain_days: u64) -> Result<()> {
        ...
        self.manifest.trash.push(bitting_format::TrashedEntry {
            entry,
    ...
    pub fn change_passphrase(
        ...
        params.validate()?;
        let mut salt = [0u8; bitting_crypto::SALT_LEN];
        bitting_crypto::fill_random(&mut salt)?;
...
pub fn calibrate(budget: std::time::Duration) -> Result<bitting_crypto::Calibration> {
    let cores = std::thread::available_parallelism()
        .map(std::num::NonZeroUsize::get)
        .unwrap_or(4);
    Ok(bitting_crypto::calibrate(
        budget,
        0,
