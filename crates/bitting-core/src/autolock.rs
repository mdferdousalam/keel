...
//!
//! **Idle means idle in Bitting, not idle at the OS.** A user reading a long document is
...
pub enum LockReason {
    /// No interaction with Bitting for the idle timeout.
    Idle,
    /// The screen locked.
    ScreenLocked,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Event {
    /// The user did something with Bitting.
    Activity,
    /// The screen locked.
    ScreenLocked,
    /// The machine is about to suspend.
