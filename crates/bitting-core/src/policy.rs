...
pub use bitting_format::manifest::{ClientKind, PersistedGrant, Scope};
...
pub enum Operation {
    ...
    ///
    /// The most dangerous operation Bitting has: it produces, in one place, exactly the file
    ...
    fn evaluate(
        ...
        if operation.requires_human_client() && !client.client_type.is_human_driven() {
            return Decision::deny(format!(
                "the {} cannot perform {}; it reads every record, so it is available \
                 only from the Bitting app or the command line",
        ...
        if let Operation::RevealSecret { entry, reason } = operation {
            ...
            if !client.client_type.can_prompt_user() {
                ...
                // now (`bitting approvals`), and a headless machine with a person at a terminal
        ...
        match e.check(&c, &reveal(ENTRY), &[], NOW) {
            Decision::Ask(request) => {
                ...
                assert_eq!(
                    ...
                    Some("/usr/local/bin/bitting-mcp")
                );
            }
