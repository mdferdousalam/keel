...
//!    [`bitting_crypto::strength`](bitting_crypto::strength) for exactly how much that
...
//! the single most secret-exposing operation Bitting performs. Two consequences are
...
use bitting_crypto::strength::Strength;
use bitting_format::manifest::EntryMeta;
use bitting_format::Id;
...
pub fn assess(candidates: &[Candidate<'_>], now: u64) -> Report {
    ...
    let mut key = [0u8; 32];
    let keyed = bitting_crypto::fill_random(&mut key).is_ok();
    ...
    for candidate in candidates {
        ...
        let bits = bitting_crypto::strength::estimate_bits(password);
        let age_days = now
