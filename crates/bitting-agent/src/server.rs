...
//! `bitting_core::policy`, so there is one place to read rather than one per request type.

use std::sync::{Arc, Mutex};

use bitting_core::audit::Outcome;
use bitting_core::autolock::LockReason;
use bitting_core::policy::{Client, Decision, Destination, Operation};
use bitting_core::EntryDraft;
use bitting_format::manifest::Id;
use bitting_format::RecordBody;
use bitting_proto::{
...
pub const IDLE_EXIT_ENV: &str = "BITTING_AGENT_IDLE_EXIT_SECS";
...
impl Agent {
    ...
    #[must_use]
    pub fn new(paths: VaultPaths, hardening: bitting_hardening::HardeningReport) -> Self {
        Self {
    ...
    fn start_watchdog(&self) {
        ...
        let _ = std::thread::Builder::new()
            .name("bitting-watchdog".to_owned())
            .spawn(move || {
                ...
                loop {
                    ...
                    if let Some(reason) = guard.enforce_autolock() {
                        eprintln!("bitting-agent: locked ({})", reason.message());
                    }

                    let is_locked =
                        !matches!(guard.lock_state(), bitting_proto::LockState::Unlocked);
                    drop(guard);
            ...
            match listener.accept() {
                ...
                Err(error) => {
                    ...
                    if matches!(error, crate::transport::TransportError::Io { .. }) {
                        eprintln!("bitting-agent: accept failed: {error}");
                        return;
                    }
                    eprintln!("bitting-agent: connection refused: {error}");
                }
            }
        }
    }
...
impl Session {
    ...
    fn handle_export(&mut self, client: &Client, passphrase: &str) -> Result<Response> {
        ...
        let metas: Vec<bitting_format::manifest::EntryMeta> = state.vault()?.entries().to_vec();
        ...
        for meta in &metas {
            let body = state
                ...
                .map_err(|e| Failure::from_core(&e))?;
            entries.push(bitting_proto::ExportedEntry {
    ...
    fn handle_audit_tail(&mut self, client: &Client, limit: Option<u32>) -> Result<Response> {
        ...
        let report =
            bitting_core::audit::read_log(&key, &bytes).map_err(|e| Failure::from_core(&e))?;
        ...
        let integrity = bitting_core::audit::check_against_anchor(&report, state.audit_anchor());
        ...
        let records = report
            ...
            .unwrap_or(&[])
            .iter()
            .map(|record| bitting_proto::AuditEntry {
                seq: record.seq,
                timestamp: record.timestamp,
                client_id: record.client_id.clone(),
                operation: record.operation.clone(),
                outcome: record.outcome.label().to_owned(),
                entry: record.entry.as_ref().map(bitting_proto::id_to_hex),
            })
            .collect();
        ...
        Ok(Response::Audit {
            ...
            chain: match integrity {
                bitting_core::audit::ChainIntegrity::Intact => bitting_proto::ChainState::Intact,
                bitting_core::audit::ChainIntegrity::BrokenAt { seq } => {
                    bitting_proto::ChainState::BrokenAt { seq }
                }
                bitting_core::audit::ChainIntegrity::Truncated { after_seq } => {
                    bitting_proto::ChainState::TruncatedAfter { seq: after_seq }
                }
                bitting_core::audit::ChainIntegrity::TailAltered {
                    expected_seq,
                    found_seq,
                } => bitting_proto::ChainState::TailAltered {
                    expected_seq,
                    found_seq,
                },
            },
    ...
    fn handle_vault_health(&mut self, client: &Client) -> Result<Response> {
        ...
        let candidates: Vec<bitting_core::health::Candidate<'_>> = bodies
            .iter()
            .filter_map(|(index, body)| {
                metas
                    .get(*index)
                    .map(|meta| bitting_core::health::Candidate {
                        meta,
        ...
        let mut report = bitting_core::health::assess(&candidates, crate::state::now());
    ...
    fn handle_grant(
        ...
        let filter = match &tag_filter {
            Some(pattern) => bitting_core::policy::EntryFilter::TagGlob(pattern.clone()),
            None => bitting_core::policy::EntryFilter::All,
        };

        let mut grant_id = [0u8; 16];
        bitting_crypto::fill_random(&mut grant_id).map_err(|_| {
            Failure::new(
                ErrorCode::Internal,
        ...
        let ttl = ttl_secs.unwrap_or(bitting_core::policy::DEFAULT_GRANT_TTL);
        let now = crate::state::now();
        let grant = bitting_core::policy::Grant::new(grant_id, target, parsed, filter, now, ttl);
        ...
        Ok(Response::Grants {
            grants: vec![bitting_proto::GrantSummary {
                client_id: target.to_owned(),
                scopes: scopes.to_vec(),
                tag_filter,
                expires_at,
                uses_remaining: bitting_core::policy::DEFAULT_MAX_USES,
            }],
        })
    }
    ...
    fn handle_list_grants(&mut self, client: &Client) -> Result<Response> {
        ...
        let grants = state
            ...
            .persistable(now)
            .into_iter()
            .map(|g| bitting_proto::GrantSummary {
    ...
    fn handle_resolve_approval(
        ...
        if let Some((asking_client, entry, operation)) = answered {
            ...
            state.audit_as(
                &asking_client,
                bitting_core::policy::ClientType::Gui,
                operation,
                entry,
                outcome,
            );
    ...
    fn handle_candidates(&mut self, client: &Client, origin: &str) -> Result<Response> {
        ...
        let mut state = self.lock_state()?;
        state.touch();
        let entries: Vec<bitting_format::manifest::EntryMeta> = state.vault()?.entries().to_vec();
        let matches = bitting_core::origin::candidates_for(&entries, &request_origin);
    ...
    ///    stored origins under the rules in [`bitting_core::origin`] — no wildcards, no
    ...
    fn handle_fill(
        ...
        let claims_origin = meta.origins.iter().any(|raw| {
            bitting_core::origin::Origin::parse(raw)
        ...
        if !claims_origin {
            ...
            return Err(Failure::new(
                ErrorCode::Denied,
                format!(
                    "that entry is not stored for {} — Bitting will not fill a password into a \
    ...
    fn require_browser(client: &Client) -> Result<()> {
        if client.client_type == bitting_core::policy::ClientType::Extension {
            return Ok(());
        }
        Err(Failure::new(
            ErrorCode::Denied,
            "only the Bitting browser extension may ask about page origins or fill credentials",
        ))
    }

    /// Show a secret in the native overlay.
    ///
    /// The agent spawns `bitting-reveal` and writes the value to its stdin, so the plaintext goes
    ...
    fn handle_reveal_on_screen(
        ...
        let operation = Operation::RevealSecret {
            entry: id,
            reason: "shown in the Bitting overlay window".to_owned(),
        };
        self.authorize(client, &operation, Some(id))?;
        ...
        let label = state
            ...
            .unwrap_or_else(|_| "Bitting".to_owned());
        drop(body);
    ...
    fn handle_read_settings(&mut self, client: &Client) -> Result<Response> {
        ...
        Ok(Response::Settings(Box::new(bitting_proto::SettingsView {
    ...
    fn handle_pending_approvals(&mut self, client: &Client) -> Result<Response> {
        ...
        for item in pending {
            ...
            approvals.push(bitting_proto::PendingApprovalView {
                approval_id: item.id,
    ...
    fn authorize(
            ...
            .check(client, operation, &tags, crate::state::now())
        {
            ...
            Decision::Ask(_) => Err(Failure::new(
                ErrorCode::Denied,
                "this operation needs your approval in the Bitting window",
            )),
        }
    }
    ...
    fn resolve_destination(&self, action: &SecretAction) -> Result<Destination> {
        match action {
            ...
            SecretAction::FillInBrowser { origin } => {
                ...
                Err(Failure::new(
                    ErrorCode::Denied,
                    "browser fill is performed by the Bitting extension, which supplies the page \
...
fn parse_scope(name: &str) -> Result<bitting_core::policy::Scope> {
    use bitting_core::policy::Scope;
...
const fn scope_name(scope: bitting_core::policy::Scope) -> &'static str {
    use bitting_core::policy::Scope;
    match scope {
...
fn health_entries(
    ...
    entries: &[bitting_core::health::EntryHealth],
) -> Result<Vec<bitting_proto::HealthEntry>> {
    entries
        .iter()
        .map(|entry| {
            Ok(bitting_proto::HealthEntry {
...
///
/// The binary is looked for beside the agent, so an installed Bitting uses its own overlay rather
...
#[cfg(windows)]
const REVEAL_BINARY: &str = "bitting-reveal.exe";
/// Filename of the overlay binary, as installed next to the agent.
#[cfg(not(windows))]
const REVEAL_BINARY: &str = "bitting-reveal";
...
fn parse_fill_origin(raw: &str) -> Result<bitting_core::origin::Origin> {
    bitting_core::origin::Origin::parse(raw).ok_or_else(|| {
        Failure::new(
            ErrorCode::BadRequest,
            "that is not an origin Bitting will fill into; only http and https pages are \
...
fn generate(
    length: Option<u32>,
    words: Option<u32>,
    defaults: Option<bitting_format::GeneratorDefaults>,
) -> Result<(String, f64)> {
    if let Some(words) = words {
        let policy = bitting_crypto::PassphrasePolicy {
            words: words as usize,
            separator: '-',
            capitalize: false,
        };
        let phrase = bitting_crypto::generate_passphrase(&policy)
    ...
    let mut policy = defaults
        .map(bitting_format::GeneratorDefaults::to_policy)
        .unwrap_or_default();
    if let Some(length) = length {
        policy.length = length as usize;
    }
    let password = bitting_crypto::generate_password(&policy)
        .map_err(|e| Failure::new(ErrorCode::BadRequest, e.to_string()))?;
...
fn apply_secret(
    ...
    match destination {
        ...
        Destination::FillInBrowser { .. } => Err(Failure::new(
            ErrorCode::Internal,
            "browser fill needs the Bitting browser extension, which is not connected",
        )),
    }
}
...
pub fn run(paths: VaultPaths) -> std::result::Result<(), crate::transport::TransportError> {
    ...
    let hardening = bitting_hardening::init();
    for warning in hardening.warnings() {
        eprintln!("bitting-agent: {warning}");
    }

    let path = crate::transport::socket_path();
    let listener = Listener::bind(&path)?;
    eprintln!("bitting-agent: listening on {}", listener.path().display());
...
mod tests {
    ...
    fn a_provided_empty_password_is_refused() {
        ...
        let state = AgentState::new(
            VaultPaths::new("/tmp/does-not-exist/vault.bitting").unwrap(),
            bitting_hardening::init(),
        );
