...
use bitting_proto::{decode_frame, encode_frame, FrameError, MAX_FRAME_LEN};
...
pub fn socket_path() -> PathBuf {
    ...
    #[cfg(unix)]
    {
        ...
        std::env::temp_dir()
            .join(format!("bitting-{}", current_uid()))
            .join("agent.sock")
    }
    #[cfg(not(unix))]
    {
...
fn directories_data_dir() -> Option<PathBuf> {
    ...
    #[cfg(target_os = "linux")]
    {
        std::env::var("XDG_DATA_HOME")
            ...
            .map(|base| base.join("bitting"))
    }
    #[cfg(not(any(target_os = "macos", target_os = "linux")))]
    {
        std::env::var("APPDATA")
            .ok()
            .map(|base| PathBuf::from(base).join("bitting"))
    }
}

/// This process's effective user id.
///
/// Delegated to `bitting-hardening`, which is the one crate permitted `unsafe` and therefore
/// the only place a raw `geteuid` belongs.
#[must_use]
pub fn current_uid() -> u32 {
    bitting_hardening::platform::current_uid()
}
...
pub struct PeerIdentity {
    ...
    /// be `bitting-mcp` from an unexpected location is visible to a human. The lookup is
...
impl Listener {
            ...
            if !is_socket {
                return Err(TransportError::io(
                    ...
                    std::io::Error::new(
                        ...
                        "refusing to remove it: BITTING_AGENT_SOCKET may be pointing at a real file",
                    ),
                ));
            }
    ...
    pub fn bind(_path: &Path) -> Result<Self> {
        Err(TransportError::Unsupported(
            "the Bitting agent does not yet support this platform; a Windows named-pipe \
    ...
    pub fn accept(&self) -> Result<Connection> {
        ...
        let peer = bitting_hardening::platform::peer_credentials(&stream)
...
/// reasoning as `bitting_client`'s `handshake`: keeping the crate compiling on Windows means the
...
fn executable_for_pid(pid: u32) -> Option<String> {
    ...
    #[cfg(not(target_os = "linux"))]
    {
        ...
        // bitting-hardening and is not implemented yet. The approval dialog copes with
...
mod tests {
    use super::*;
    use bitting_proto::{Request, Response};
    ...
    // is one that permits data loss, since `BITTING_AGENT_SOCKET` is user-settable and a typo
    ...
    fn the_socket_path_can_be_overridden_for_tests_and_multiple_vaults() {
        ...
        std::env::set_var(SOCKET_ENV, "/tmp/bitting-override-test.sock");
        assert_eq!(
            socket_path(),
            PathBuf::from("/tmp/bitting-override-test.sock")
        );
        match previous {
    ...
    fn binding_refuses_a_symlink_at_the_socket_path() {
        ...
        // it would delete whatever it pointed at, and `BITTING_AGENT_SOCKET` is user-settable.
        let dir = tempfile::tempdir().expect("temp dir");
