...
use bitting_core::audit::{AuditEvent, AuditLog, Outcome};
use bitting_core::autolock::{AutoLock, Event as LockEvent, LockPolicy, LockReason};
use bitting_core::policy::{Client, ClientType, PolicyEngine};
use bitting_core::{OpenOptions, UnlockFactors, UnlockedVault};
use bitting_crypto::{subkeys, SecretString};
use bitting_format::manifest::Id;
use bitting_proto::{ClientKind, EntryRef, ErrorCode, LockState};
...
impl Failure {
    ...
    pub fn from_core(error: &bitting_core::Error) -> Self {
        use bitting_core::Error as E;
        match error {
            E::Unlock => Self::new(
            ...
            E::Store(bitting_store::Error::NotFound(_)) => {
                Self::new(ErrorCode::NoVault, "no vault exists at that path")
            }
            E::Store(bitting_store::Error::AlreadyExists(_)) => Self::new(
                ErrorCode::VaultExists,
                "a vault already exists at that path",
            ),
            E::Store(bitting_store::Error::ConcurrentModification) => Self::new(
                ErrorCode::Conflict,
                "the vault changed on disk since it was loaded; reload before saving",
            ),
            E::Store(bitting_store::Error::AlreadyLocked) => Self::new(
                ErrorCode::Conflict,
                "another Bitting instance has this vault open",
            ),
...
pub struct AgentState {
    ...
    gui_sessions: usize,
    hardening: bitting_hardening::HardeningReport,
...
impl AgentState {
    ...
    #[must_use]
    pub const fn hardening(&self) -> &bitting_hardening::HardeningReport {
        &self.hardening
    }
    ...
    pub fn create_vault(&mut self, passphrase: &str, tier: Option<&str>) -> Result<()> {
        ...
        let params = bitting_core::tier_params(tier);
    ...
    pub fn handle_for(&mut self, id: &Id) -> Result<EntryRef> {
        ...
        let mut raw = [0u8; 16];
        bitting_crypto::fill_random(&mut raw).map_err(|_| {
            Failure::new(
                ErrorCode::Internal,
                "the operating system random number generator failed",
            )
        })?;
        let handle = bitting_proto::id_to_hex(&raw);
    ...
    pub fn audit_as(
        &mut self,
        client_id: &str,
        client_type: bitting_core::policy::ClientType,
        operation: &str,
        entry: Option<Id>,
        outcome: Outcome,
    ) {
    ...
    pub fn save_vault(&mut self) -> Result<()> {
        ...
        let anchor = self
            .audit
            .as_ref()
            .map(|log| bitting_format::manifest::AuditAnchor {
    ...
    #[must_use]
    pub fn audit_anchor(&self) -> Option<bitting_format::manifest::AuditAnchor> {
    ...
    pub fn audit_key(&self) -> Result<bitting_crypto::Key256> {
    ...
    pub fn flush_audit(&mut self) {
        ...
        // the user has performed, and the file's existence reveals that they use Bitting at
...
/// where `bitting log` reports it. Deliberately not "reset on a bad log": silently starting a
...
fn resume_audit_log(path: &std::path::Path, key: bitting_crypto::Key256) -> AuditLog {
    ...
    match bitting_core::audit::read_log(&key, &bytes) {
        Ok(report) => {
...
fn derive_audit_key(vault: &UnlockedVault) -> bitting_core::Result<bitting_crypto::Key256> {
...
fn build_factors(passphrase: &str, keyfile: Option<Vec<u8>>) -> Result<UnlockFactors> {
    ...
    if passphrase.len() > bitting_crypto::MAX_PASSPHRASE_LEN {
        return Err(Failure::new(
            ErrorCode::BadRequest,
            "that passphrase is longer than Bitting accepts",
        ));
    }
...
fn parse_tier(name: Option<&str>) -> Result<bitting_crypto::KdfTier> {
    match name.map(str::to_ascii_lowercase).as_deref() {
        None | Some("balanced") => Ok(bitting_crypto::KdfTier::Balanced),
        Some("interactive") => Ok(bitting_crypto::KdfTier::Interactive),
        Some("paranoid") => Ok(bitting_crypto::KdfTier::Paranoid),
...
mod tests {
    ...
    fn tier_names_parse_and_unknown_ones_are_rejected() {
        assert_eq!(parse_tier(None).unwrap(), bitting_crypto::KdfTier::Balanced);
        assert_eq!(
            parse_tier(Some("PARANOID")).unwrap(),
            bitting_crypto::KdfTier::Paranoid
        );
        assert_eq!(
            parse_tier(Some("interactive")).unwrap(),
            bitting_crypto::KdfTier::Interactive
        );
    ...
    fn an_empty_or_oversized_passphrase_is_rejected() {
        ...
        let huge = "x".repeat(bitting_crypto::MAX_PASSPHRASE_LEN + 1);
        assert_eq!(
    ...
    fn every_unlock_failure_reports_the_same_thing() {
        ...
        let failure = Failure::from_core(&bitting_core::Error::Unlock);
    ...
    fn store_errors_map_onto_actionable_codes() {
        use bitting_store::Error as S;
        assert_eq!(
            Failure::from_core(&bitting_core::Error::Store(S::ConcurrentModification)).code,
            ErrorCode::Conflict
        );
        assert_eq!(
            Failure::from_core(&bitting_core::Error::Store(S::AlreadyLocked)).code,
            ErrorCode::Conflict
        );
        assert_eq!(
            Failure::from_core(&bitting_core::Error::Locked).code,
            ErrorCode::Locked
        );
        assert_eq!(
            Failure::from_core(&bitting_core::Error::NoSuchEntry).code,
            ErrorCode::NotFound
        );
    }
