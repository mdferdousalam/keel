...
use bitting_crypto::kdf::Argon2Params;
use bitting_crypto::{Nonce, Sealed, AEAD_ID_XCHACHA20POLY1305, KDF_ID_ARGON2ID_V13, NONCE_LEN};
...
pub const MAGIC: [u8; 8] = *b"BITTVLT\x01";
...
pub const FOOTER_MAGIC: [u8; 8] = *b"BITTEND\x01";
...
const AAD_WRAP: &[u8] = b"bitting/v1/wrap";
/// Domain string for manifest associated data.
const AAD_MANIFEST: &[u8] = b"bitting/v1/manifest";
/// Domain string for record associated data.
const AAD_RECORD: &[u8] = b"bitting/v1/record";
...
pub struct Header {
    ...
    /// Per-vault KDF salt.
    pub kdf_salt: [u8; bitting_crypto::SALT_LEN],
...
impl Header {
    ...
    pub fn identity_hash(&self) -> [u8; 32] {
        ...
        hasher.update(b"bitting/v1/identity");
    ...
    pub fn decode(buf: &[u8]) -> Result<(Self, usize)> {
        ...
        if salt_len != bitting_crypto::SALT_LEN {
            return Err(Error::Corrupt("KDF salt has an unexpected length"));
        }
        let kdf_salt = r.array::<{ bitting_crypto::SALT_LEN }>()?;
...
mod tests {
    use super::*;
    use bitting_crypto::kdf::MIN_M_COST_KIB;
    ...
    fn rejects_absurd_kdf_parameters_from_a_hostile_file() {
        ...
        assert!(matches!(
            Header::decode(&bytes),
            Err(Error::Crypto(bitting_crypto::Error::KdfParams(_)))
        ));
    }
    ...
    fn record_and_manifest_aad_survive_a_passphrase_change() {
        ...
        after.kdf_salt = [0xEE; bitting_crypto::SALT_LEN];
