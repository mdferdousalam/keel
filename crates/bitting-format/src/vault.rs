...
use bitting_crypto::{aead, Key256, Sealed, NONCE_LEN};
...
/// key, which `bitting-core` derives from the vault master key.
///
...
pub fn parse(bytes: &[u8]) -> Result<ParsedVault<'_>> {
    ...
    if header.manifest_len < (NONCE_LEN + bitting_crypto::TAG_LEN) as u64 {
...
mod tests {
    ...
    use bitting_crypto::{subkeys, SecretBytes, AEAD_ID_XCHACHA20POLY1305, KDF_ID_ARGON2ID_V13};
