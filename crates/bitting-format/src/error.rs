...
//!
//! Note the distinction from [`bitting_crypto::Error::Unlock`], which deliberately
...
pub enum Error {
    /// The file does not start with the Bitting magic number.
    #[error("not a Bitting vault file")]
    BadMagic,

    /// The format version is newer than this build understands.
    ///
    /// Kept separate from [`Error::Corrupt`] so the message can be actionable:
    /// the user needs a newer Bitting, not a backup.
    #[error("vault format version {found} is newer than this version of Bitting supports (max {supported})")]
    UnsupportedVersion {
    ...
    #[error(transparent)]
    Crypto(#[from] bitting_crypto::Error),
