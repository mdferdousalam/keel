...
mod tests {
    ...
    fn round_trips_through_the_writer() {
        // Bound to a named payload rather than a literal length. The read used to be
        // `r.take(4)`, where the 4 was the length of the payload written above it — so
        // changing that payload broke the test somewhere it did not mention.
        const TAIL: &[u8] = b"bitting";
        ...
        assert_eq!(r.take(TAIL.len()).unwrap(), TAIL);
        assert!(r.is_empty());
    }
