...
///
/// Defined here rather than in `bitting-core` because grants are persisted in the
/// manifest, and the on-disk schema belongs with the format. `bitting-core` builds its
...
///
/// Mirrors `bitting_crypto::PasswordPolicy` rather than reusing it, so that
/// `bitting-crypto` stays free of a serde dependency and no secret-adjacent type in
...
impl GeneratorDefaults {
    /// Convert into a `bitting-crypto` policy.
    #[must_use]
    pub fn to_policy(self) -> bitting_crypto::PasswordPolicy {
        bitting_crypto::PasswordPolicy {
...
mod tests {
    ...
    fn generator_defaults_convert_to_a_valid_policy() {
        ...
        bitting_crypto::generate_password(&policy).unwrap();
    }
}
