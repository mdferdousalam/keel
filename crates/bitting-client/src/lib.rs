...
//! Talking to the Bitting agent.
//!
//! This crate deliberately depends on **nothing but `bitting-proto`**. It has no access to
//! `bitting-crypto`, `bitting-format`, or `bitting-core`, and `cargo xtask check-layering` fails the
...
//! user typing `bitting get github` never has to know a daemon exists. The *first* command of
...
//! web page triggered is a phishing vector, so the extension asks the user to open Bitting
//! instead.
...
pub const SOCKET_ENV: &str = "BITTING_AGENT_SOCKET";

/// Environment variable naming the agent binary, for tests and unusual installs.
pub const AGENT_BINARY_ENV: &str = "BITTING_AGENT_BINARY";
...
pub enum Error {
    ...
    #[error("could not reach the Bitting agent: {0}")]
    Unreachable(String),
...
/// than shared: `bitting-client` cannot depend on `bitting-agent` without inverting the layering
...
pub fn socket_path() -> PathBuf {
    ...
    #[cfg(unix)]
    {
        if let Ok(runtime) = std::env::var("XDG_RUNTIME_DIR") {
            if !runtime.is_empty() {
                return PathBuf::from(runtime).join("bitting").join("agent.sock");
            }
        }
    }
    #[cfg(target_os = "macos")]
    ...
    std::env::temp_dir().join("bitting").join("agent.sock")
}
...
impl Client {
    ...
    fn attach(_path: &Path, _kind: ClientKind, _client_id: &str) -> Result<Self> {
        Err(Error::Unsupported(
            "the Bitting client does not yet support this platform",
        ))
    }
...
fn spawn_agent(vault: Option<&Path>) -> Result<()> {
    ...
    if let Some(vault) = vault {
        // `bitting-agent` takes the vault path as its first argument, falling back to
        // `BITTING_VAULT`. Passing it here leaves this process's environment untouched.
        command.arg(vault);
    }
    command
...
///
/// Looks beside our own executable first, so an installed Bitting uses its own agent rather
...
mod tests {
    ...
    fn an_explicit_socket_path_wins() {
        ...
        std::env::set_var(SOCKET_ENV, "/tmp/bitting-client-test.sock");
        assert_eq!(
            socket_path(),
            PathBuf::from("/tmp/bitting-client-test.sock")
        );
        match previous {
    ...
    fn connecting_with_nothing_listening_names_the_path() {
        ...
        let result = Client::attach(
            Path::new("/tmp/bitting-definitely-not-listening.sock"),
            ClientKind::Cli,
            "test",
        );
        match result {
            Err(Error::Unreachable(message)) => {
                assert!(
                    message.contains("bitting-definitely-not-listening"),
                    "{message}"
                );
            }
