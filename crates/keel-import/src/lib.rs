...
//!
//! Keel deliberately prefers the browser's own export over prising open its password store.
...
//! answer: it works everywhere, it needs no privileged code in Keel, and the user can see
...
pub enum Error {
    ...
    #[error(
        "could not recognise the columns in this file. Keel expects a header row naming a \
    ...
    /// The file has more rows than Keel will accept.
