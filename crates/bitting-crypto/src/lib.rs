...
pub fn self_check() -> Result<()> {
    ...
    let sealed = seal(&key, b"self-check", b"bitting")?;
    let opened = open_sealed(&key, b"self-check", &sealed)?;
    if opened.as_slice() != b"bitting" {
...
mod tests {
    ...
    fn end_to_end_key_hierarchy() {
        ...
        let wrapped = seal(&kek, b"bitting/v1/wrap-test", vmk.expose()).unwrap();

        // Unlock: rederive the KEK and unwrap.
        let kek2 = derive_kek_from_factors(&vault_uuid, &factors, &salt, params).unwrap();
        let unwrapped = open_sealed(&kek2, b"bitting/v1/wrap-test", &wrapped).unwrap();
