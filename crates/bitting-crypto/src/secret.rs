...
//!    [`PageLocker`] hook, which `bitting-hardening` installs at startup. When no
...
///
/// Implemented by `bitting-hardening` (the only crate in the workspace permitted
...
///
/// Call once, from `bitting-hardening`, before any secret exists. Returns `false`
