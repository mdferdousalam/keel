...
//!
//! Runs the real `bitting-native-host` binary over real stdio against a real agent, speaking the
...
impl Fixture {
    ...
    fn add_entry(&self, title: &str, url: &str, password: &str) {
        ...
        assert!(child.wait().expect("bitting add").success());
    }
}
...
impl Bridge {
    fn start(fixture: &Fixture) -> Self {
        let child = fixture
            .command("bitting-native-host")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::null())
            .spawn()
            .expect("spawn bitting-native-host");
        Self { child }
    }
