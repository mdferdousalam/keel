...
///
/// The lock is advisory, which is worth being explicit about: it coordinates Keel
...
impl VaultLock {
    ...
    ///
    /// Non-blocking on purpose: waiting would hang the UI, and telling the user "Keel
...
pub enum WriteMode {
    ...
    ///
    /// Guards against `keel init` silently destroying an existing vault, which would
    /// be unrecoverable.
    Create,
...
/// Shift `vault.keel` into `.bak.1`, and each backup one position older.
///
...
mod tests {
    ...
    fn create_refuses_to_clobber_an_existing_vault() {
        // `keel init` over a real vault would be unrecoverable, so it must be an error.
        let (_dir, paths) = setup();
    ...
    fn missing_directories_are_created() {
        let dir = tempfile::tempdir().unwrap();
        let nested = dir.path().join("a").join("b").join("vault.keel");
