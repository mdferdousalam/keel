...
pub enum Error {
    ...
    ///
    /// Distinct from a generic I/O error so the UI can say "Bitting is already open"
    /// rather than something alarming.
    #[error("another Bitting instance has this vault open")]
    AlreadyLocked,
    ...
    #[error(transparent)]
    Format(#[from] bitting_format::Error),
}
