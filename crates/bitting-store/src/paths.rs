...
//! | `vault.bitting.lock` | Advisory lock held during a write |
//!
...
pub const DEFAULT_VAULT_NAME: &str = "vault.bitting";
...
impl VaultPaths {
    ...
    /// | Platform | Location |
    /// |---|---|
    /// | Linux | `$XDG_DATA_HOME/bitting/vault.bitting` |
    /// | macOS | `~/Library/Application Support/dev.bitting/vault.bitting` |
    /// | Windows | `%APPDATA%\bitting\data\vault.bitting` |
    pub fn default_location() -> Result<Self> {
        let dirs = directories::ProjectDirs::from("dev", "", "bitting").ok_or_else(|| {
...
///   Bitting will still *detect* the counter regression and warn, but the attacker gets
///   to try.
/// * **Conflict copies duplicate secrets.** A `vault (conflicted copy).bitting` is a
...
mod tests {
    ...
    fn paths() -> VaultPaths {
        VaultPaths::new("/home/ada/.local/share/bitting/vault.bitting").unwrap()
    }
    ...
    fn companion_paths_derive_from_the_vault_path() {
        ...
        assert_eq!(
            p.lock(),
            Path::new("/home/ada/.local/share/bitting/vault.bitting.lock")
        );
        assert_eq!(
            p.state(),
            Path::new("/home/ada/.local/share/bitting/vault.bitting.state")
        );
        assert_eq!(
            p.audit(),
            Path::new("/home/ada/.local/share/bitting/vault.bitting.audit")
        );
        assert_eq!(
            p.backup(1),
            Path::new("/home/ada/.local/share/bitting/vault.bitting.bak.1")
        );
    }
    ...
    fn a_renamed_vault_gets_renamed_companions() {
        ...
        let a = VaultPaths::new("/vaults/work.bitting").unwrap();
        let b = VaultPaths::new("/vaults/personal.bitting").unwrap();
    ...
    fn detects_the_common_cloud_sync_folders() {
        let cases = [
            (
                "/Users/ada/Library/Mobile Documents/com~apple~CloudDocs/vault.bitting",
                CloudProvider::ICloud,
            ),
            (
                "/Users/ada/Dropbox/bitting/vault.bitting",
                CloudProvider::Dropbox,
            ),
            ("/Users/ada/OneDrive/vault.bitting", CloudProvider::OneDrive),
            (
                "/Users/ada/Google Drive/vault.bitting",
                CloudProvider::GoogleDrive,
            ),
            (
                "/home/ada/Syncthing/vault.bitting",
                CloudProvider::Syncthing,
            ),
        ];
    ...
    fn ordinary_locations_are_not_flagged() {
        for path in [
            "/home/ada/.local/share/bitting/vault.bitting",
            "/Users/ada/Library/Application Support/dev.bitting/vault.bitting",
            "C:\\Users\\ada\\AppData\\Roaming\\bitting\\vault.bitting",
        ] {
            assert_eq!(
                detect_cloud_sync(Path::new(path)),
                None,
    ...
    fn cloud_detection_is_case_insensitive() {
        assert_eq!(
            detect_cloud_sync(Path::new("/Users/ada/DROPBOX/vault.bitting")),
