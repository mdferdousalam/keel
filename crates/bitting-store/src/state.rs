...
//! # Why this matters
//!
//! Every other integrity check in Bitting verifies that a file is internally consistent.
...
//!
//! Detection therefore has to come from state held *outside* the file. Bitting keeps it
...
use bitting_format::Header;

use crate::error::{Error, Result};

/// Magic number for the state sidecar.
const STATE_MAGIC: [u8; 8] = *b"BITTSTA\x01";
...
impl LastSeen {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        ...
        if magic != STATE_MAGIC {
            return Err(Error::BadState("not a Bitting state file"));
        }
...
mod tests {
    ...
    use bitting_crypto::{AEAD_ID_XCHACHA20POLY1305, KDF_ID_ARGON2ID_V13, NONCE_LEN};
    use bitting_format::header::{WrappedKey, WRAPPED_KEY_CT_LEN};
    use bitting_format::{FactorSet, HeaderFlags};

    fn header(uuid: u8, counter: u64) -> Header {
        Header {
            format_version: bitting_format::FORMAT_VERSION,
    ...
    fn state_survives_a_file_round_trip() {
        ...
        let path = dir.path().join("vault.bitting.state");
        let s = seen(3, 77, 8);
        s.save(&path).unwrap();
