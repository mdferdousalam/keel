...
//! code with abort hooks. Nothing in `bitting-store` knows it is being tested.
...
use bitting_store::{read_vault, write_vault, VaultPaths, WriteMode};

/// Environment variable that turns this binary into the crash-test child.
#[cfg(unix)]
const CHILD_ENV: &str = "BITTING_CRASH_TEST_DIR";

/// Prefix every payload starts with.
///
/// The generation number is located by the length of this string, so both must be named
/// rather than written inline. They used to be a literal `13` in two separate places,
/// which silently encoded the old project name's length: renaming to something longer
/// moved the digits without moving either slice. One of the two was updated and the other
/// was not, and the test then failed by reporting that writes were not progressing —
/// blaming the code under test for what was a defect in the harness. Hence exactly one
/// function below knows where the digits are.
const PAYLOAD_PREFIX: &str = "BITTINGTEST-GEN-";

/// Width of the zero-padded generation number that follows the prefix.
...
fn payload(generation: usize) -> Vec<u8> {
    let mut v = format!("{PAYLOAD_PREFIX}{generation:0GENERATION_DIGITS$}-").into_bytes();
    // Pad to something big enough to span several filesystem blocks, so a torn write
    // has room to actually be torn.
    v.resize(64 * 1024, b'.');
    v.extend_from_slice(format!("-END-{generation:0GENERATION_DIGITS$}").as_bytes());
    v
}

/// The generation digits from a payload, if the bytes begin with a well-formed header.
///
/// The single place that knows where the number sits. Both the intactness check and the
/// progress check read it through here.
fn generation_field(bytes: &[u8]) -> Option<&str> {
    if !bytes.starts_with(PAYLOAD_PREFIX.as_bytes()) {
        return None;
    }
    let start = PAYLOAD_PREFIX.len();
    let digits = bytes.get(start..start + GENERATION_DIGITS)?;
    let text = std::str::from_utf8(digits).ok()?;
    text.bytes().all(|b| b.is_ascii_digit()).then_some(text)
}

/// Check that a file's contents are exactly one of the payloads we ever write.
fn is_intact_payload(bytes: &[u8]) -> bool {
    let generation: usize = match generation_field(bytes) {
...
fn a_leftover_temporary_file_does_not_affect_the_vault() {
    ...
    fs::write(
        dir.path().join(".bitting-tmp-abandoned.bitting"),
        b"half written",
    )
    .unwrap();
...
fn every_backup_is_a_complete_previous_generation() {
    ...
    for index in 1..=bitting_store::BACKUP_COUNT {
...
fn child_write_loop(dir: &Path) -> ! {
    let paths = VaultPaths::new(dir.join("vault.bitting")).unwrap();
...
fn killing_a_writer_mid_save_never_corrupts_the_vault() {
    ...
    for delay_ms in [3u64, 7, 11, 17, 23, 31, 41, 53, 67, 83] {
        ...
        if paths.exists() {
            ...
            if let Some(text) = generation_field(&bytes) {
