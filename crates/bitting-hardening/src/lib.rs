...
///
/// Surfaced in the GUI security panel and by `bitting status --json`. A user who
...
///
/// Installs the page locker into `bitting-crypto` so every `SecretBytes` and
...
pub fn init() -> HardeningReport {
    ...
    let _ = bitting_crypto::install_page_locker(locker);
