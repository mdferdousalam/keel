...
//! Raw platform calls.
//!
//! Every `unsafe` block in the Bitting workspace lives in this file and in
...
///
/// Lives here rather than in `bitting-agent` because `geteuid` is a raw libc call, and this
...
/// `unsafe`, which pushed the obligation onto `bitting-reveal` — a crate that forbids `unsafe`
