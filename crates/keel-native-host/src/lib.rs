...
//! followed by JSON; the agent speaks the Keel wire protocol. That is the entire job.
//!
...
//! could spawn an unlock prompt, then any web page could summon a Keel passphrase dialog at a
//! moment of its choosing — which is a phishing primitive, not a feature. The user opens Keel
...
use keel_proto::{ClientKind, EntryRef, Request, Response};

/// Identifier this bridge presents to the agent.
const CLIENT_ID: &str = "keel-browser";
...
const fn error_code_name(code: keel_proto::ErrorCode) -> &'static str {
    match code {
        keel_proto::ErrorCode::Locked => "locked",
        keel_proto::ErrorCode::UnlockFailed => "unlock_failed",
        keel_proto::ErrorCode::NoVault => "no_vault",
        keel_proto::ErrorCode::VaultExists => "vault_exists",
        keel_proto::ErrorCode::NotFound => "not_found",
        keel_proto::ErrorCode::Denied => "denied",
        keel_proto::ErrorCode::RateLimited => "rate_limited",
        keel_proto::ErrorCode::ApprovalRefused => "approval_refused",
        keel_proto::ErrorCode::BadRequest => "bad_request",
        keel_proto::ErrorCode::VaultDamaged => "vault_damaged",
        keel_proto::ErrorCode::Conflict => "conflict",
        keel_proto::ErrorCode::Internal => "internal",
    }
}
    ...
    loop {
        ...
        if client.is_none() {
            match Client::connect_existing(ClientKind::Extension, CLIENT_ID) {
                ...
                Err(_) => {
                    // The distinct code lets the extension say "open Keel" rather than
                    ...
                    let reply = ToExtension::failed(
                        id,
                        "agent_not_running",
                        "Keel is not running. Open Keel and unlock your vault.",
                    );
                    respond(&mut writer, &reply)?;
                    continue;
                }
        ...
        let reply = match client.as_mut() {
            ...
            None => ToExtension::failed(id, "agent_not_running", "Keel is not running."),
        };
        respond(&mut writer, &reply)?;
    }
}
