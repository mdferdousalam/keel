# The Bitting browser extension

Loaded unpacked during development:

1. Run `bitting setup-browser` once. It writes the native-messaging manifests that let a
   browser launch `bitting-native-host`, and prints where it put them.
2. Open `chrome://extensions`, turn on Developer mode, choose **Load unpacked**, and select
   this directory.
3. Copy the extension ID Chrome assigns and re-run `bitting setup-browser --extension-id <ID>`,
...
the toolbar button. The trade is real: Bitting cannot detect a login form and offer to fill it
...
impersonating the browser, which is outside the threat model the rest of Bitting is written
